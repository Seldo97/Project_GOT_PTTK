...
//pobieram elementy
const form = document.querySelector("#signupFormUpdate");
...
inputPassword.addEventListener("input", e => markErrorField(e.target, !testPassword(e.target)));
...
form.addEventListener("submit", e => {
    ...
    if (!testLength(inputName, 3)) {
     ...
     if (!testLength(inputLogin, 2)) {
       ...
       if (!testPassword(inputPassword) && inputPassword.value.length !== 0) {
